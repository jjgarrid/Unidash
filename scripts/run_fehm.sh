...
if [ -d "app/output/pared2d" ]; then
    ...
    cd ../FEHM/fehmpytests/escenarios/heat/pared/input
fi
...
if [ -d "app/output/pared3d" ]; then
    ...
    echo $PWD
    cd ../FEHM/fehmpytests/escenarios/heat/pared3d/input
fi
...
if [ -d "app/output/barovel" ]; then
    ...
    echo $PWD
    cd ../FEHM_mast/fehmpytests/escenarios/isotermal/baro_vel_V2/input
fi

# run FEHM
xfehm control/fehmn.files